...
class ApiUrlsTest(SimpleTestCase, PassengerTestBase, TripTestBase):
    def test_passengerIsResolved(self):
        self.assertEquals(self.resolvedPassengerUrl.func.view_class, PassengerView)

    def test_passengerDetailIsResolved(self):
        self.assertEquals(
            self.resolvedPassengerDetailUrl.func.view_class, PassengerDetailView
        )

    def test_tripIsResolved(self):
        self.assertEquals(self.resolvedTripUrl.func.view_class, TripView)

    def test_tripDetailIsResolved(self):
        self.assertEquals(self.resolvedTripDetailUrl.func.view_class, TripDetailView)
...
class PassengerTest(APITestCase, PassengerTestBase):
    def setUp(self):
        ...
        for _ in range(self.tripCount):
            Trip.objects.create(
                passenger=passengerList[random.randint(0, self.passengerCount - 1)],
                startTime=timezone.now(),
                endTime=(timezone.now() + random.random() * timedelta(days=1)),
    ...
    def test_getAll(self):
        ...
        self.assertEquals(
            response.status_code, status.HTTP_200_OK, "Status Code is not correct"
        )
        self.assertEquals(
        ...
        self.assertTrue(
            len(response.json()["passengers"]) > 0, "Returned no passengers"
        )
        self.assertEquals(
    ...
    def test_create(self):
        ...
        self.assertEquals(
            response.status_code, status.HTTP_201_CREATED, "Status Code is not correct"
        )
        self.assertEquals(
            list(response.json().keys()),
            ["passengers", "result", "errorMessage"],
...
class PassengerDetailTest(APITestCase, PassengerTestBase):
    ...
    def test_get_one(self):
        ...
        self.assertTrue(
            "id" in response.json()["passengers"].keys(),
    ...
    def test_update(self):
        ...
        self.assertTrue(
            Utility.compareObjects(data, response.json()["passengers"]),
            "Updated data is not correct",
        )
    ...
    def test_delete(self):
        ...
        self.assertEquals(
            responseDel.status_code,
            status.HTTP_204_NO_CONTENT,
            "Status code is not correct",
        )
        self.assertEquals(
            responseDel.data["result"],
            ResultTypes.DELETED,
            "Returned result is not correct",
        )
        self.assertEquals(
            responseGet.status_code, status.HTTP_404_NOT_FOUND, "Data is not deleted."
        )
...
class TripTest(APITestCase, TripTestBase):
    ...
    def test_getAll(self):
        ...
        self.assertTrue(len(response.json()["trips"]) > 0, "Returned no trips")
        self.assertEquals(
            response.json()["result"],
            ResultTypes.RETRIEVED,
            "Returned result value is not correct",
...
class TripDetailTest(APITestCase, TripTestBase):
    ...
    def test_get_one(self):
        ...
        self.assertTrue(
            "id" in response.json()["trips"].keys(),
    ...
    def test_update(self):
        data = {
            ...
            "endTime": (
                timezone.now() + random.random() * timedelta(days=1)
            ).isoformat(),
            "startLocation": self.faker.unique.city(),
            "endLocation": self.faker.unique.city(),
            "totalDistance": random.uniform(0, 500),
